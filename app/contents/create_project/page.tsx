...
// ✅ 다크/라이트에서 select(option) 글씨 가독성 문제 해결
// - globals.css 전역 수정 금지
// - 해당 select 1개에만 colorScheme=light 적용(네이티브 드롭다운 가독성 확보)
  ...
  const transferOwner = async (projectUid: string) => {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        // ✅ 새 오너는 members에 있어야 함(빈 값 제거 후 검사)
                  ...
                  <div className="mt-2 text-xs opacity-80 flex flex-wrap items-center gap-2">
                    ...
                    {memberUids.length > 0 && (
                      <span className="inline-flex items-center gap-2 border rounded px-2 py-1">
                        ...
                        <select
                          // ✅ 핵심: 다크모드에서 option이 흰배경/흰글씨로 보이는 네이티브 렌더링 이슈 해결
                          // ✅ globals.css 수정 없이, 이 select 하나만 light 스킴으로 렌더링
